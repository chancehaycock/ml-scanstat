...
from SpatialScan.likelihood import likelihood_ratio, ebp_asym
...
class NetworkScan(PathGenerator):
    ...
    def scan(self):
        ...
        unsorted_results = pd.DataFrame.from_dict(return_dict, "index")
        self.results = unsorted_results.sort_values('ebp_score', ascending=False)
...
class NetworkScanPaper(PathGenerator):
    ...
    def __init__(
        ...
        min_path_length=50,
        max_path_length=500,
        network_detectors = None,
        network_paths = None,
        detector_edges = None,
        nearest_detector_edges=None,
        **kwargs
    ):
        self.forecast = forecast
        self.borough = borough
        self.borough_file_path = "../../data/ESRI/London_Borough_Excluding_MHW.shp"
        self.network_detectors = network_detectors
        ...
        if not isinstance(network_detectors, list):
            self.network_forecast = restrict_readings_to_network(
                self.network, self.forecast
            )
            self.detector_edges = self.get_detector_edges()

        else:
            self.network_forecast = forecast[forecast['detector_id'].isin(self.network_detectors)]
            #print("Original forecast: ", self.network_forecast)
            inter_df = self.network_forecast.merge(nearest_detector_edges, on=['detector_id'])
            #print("Merged DF: ", inter_df)
            self.network_forecast = inter_df
            self.detector_edges = detector_edges

        super().__init__(
            self.network, min_path_length, max_path_length, self.detector_edges, **kwargs
        )
        logging.info(
            "Calculating paths of lengths between %f and %f",
            self.min_path_length,
        ...
        if not isinstance(network_paths, dict):
            self.generate_paths()
        else:
...
def get_borough_polygon(
    ...
    if borough == "London":
        return gpd.GeoSeries(unary_union(london_df.geometry)).iloc[0]
